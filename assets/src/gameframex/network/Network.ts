...
export default class Network {
    ...
    private _sendByte: DataView;
    private _recvByte: DataView;
    ...
    constructor() {
        this.count = 1000;
        this._sendByte = new DataView(new ArrayBuffer(1024 * 1024 * 64), 0);
        // this._sendByte.endian = Laya.Byte.BIG_ENDIAN;
        this._recvByte = new DataView(new ArrayBuffer(1024 * 1024 * 64), 0);
        // this._recvByte.endian = Laya.Byte.BIG_ENDIAN;
        // this._socket = new WebSocket();
        // this._socket.endian = Laya.Byte.BIG_ENDIAN;
       
        // cc.timer.loop(5000, this, this.onHeart);
    ...
    private openHandler(event: any = null): void {
        ...
        LogHelper.log(`连接成功: host:${this._host} port:${this._port} `);
    ...
    private checkReconnect(): void {
        ...
        // director.getScheduler().once(2000, this, this.reconnect);
    }
    //------------------------------事件结束------------------------------
    ...
    connect(host: string, port: number = 0): void {
        ...
        this._socket = new WebSocket(host + ":" + port);
        this.addEvents();
    ...
    public send(message: MessageObject): boolean {
        ...
        let offset = 0;
        this._sendByte.setUint16(offset, len); // length
        this._sendByte.setUint8(offset, 4);// operationType
        this._sendByte.setUint8(offset, 0); // zipFlag
        this._sendByte.setInt32(offset, message.UniqueId); // uniqueId
        this._sendByte.setInt32(offset, messageId); // uniqueId
    ...
    private receiveHandler(data: any = null): void {
        ...
        let pkgLen = this._recvByte.byteLength;

        if (pkgLen < 12) {
            return;
        }

        let msgTotalLength = this._recvByte.getUint16(0);
        //operationType
        let operationType = this._recvByte.getUint8(2);
        //zipFlag
        let zipFlag = this._recvByte.getUint8(3);
        let uniqueId = this._recvByte.getInt32(6);
        let MessageId = this._recvByte.getInt32(8);
